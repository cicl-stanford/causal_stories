...
```{r}
bind_rows(df.exp1.no_feedback.full %>% 
            filter(trial == "rule_question") %>% 
            select(workerid, accuracy) %>% 
            mutate(condition = "no feedback"),
          df.exp1.feedback.full %>% 
            filter(trial == "rule_question") %>% 
            select(workerid, accuracy) %>% 
            mutate(condition = "feedback"), 
          df.exp1.short.full %>% 
...
### Accuracy in first surprise task trial depending on last prediction task trial 

```{r}
fun_order_effect = function(data, condition_name){
  data = data %>% 
    mutate(end_position = ifelse(is.na(end_position), end_posision, end_position)) %>% 
    select(workerid, trial, end_position, accuracy) %>% 
    group_by(workerid, trial) %>% 
    mutate(order = 1:n()) %>% 
    ungroup() %>% 
    filter((trial == "prediction_trial" & order == 4) |
             (trial == "surprise_quiz" & order == 1)) %>% 
    select(-order) %>% 
    pivot_wider(names_from = trial,
                values_from = c(end_position, accuracy)) %>% 
    select(-accuracy_prediction_trial) %>% 
    rename(prediction = end_position_prediction_trial,
           surprise = end_position_surprise_quiz,
           accuracy_surprise = accuracy_surprise_quiz) %>% 
    mutate(condition = condition_name)
}

# only keep first 120 participants
df.exp2.short.trials_selected = df.exp2.short.surprise_quiz %>% 
  distinct(workerid) %>% 
  left_join(df.exp2.short.trials,
            by = "workerid")

df.exp2.order_effect = bind_rows(fun_order_effect(data = df.exp2.long.trials,
                                                  condition_name = "long"),
                                 fun_order_effect(data = df.exp2.short.trials_selected,
                                                  condition_name = "short"),
                                 fun_order_effect(data = df.exp2.conjunctive.trials,
                                                  condition_name = "conjunctive")) %>% 
  mutate(index = ifelse(prediction == surprise, "same", "different"),
         accuracy_surprise = ifelse(accuracy_surprise == "False", 0, 1))

# calculate overall accuracy 
df.exp2.order_effect %>% 
  group_by(condition, index) %>% 
  summarize(accuracy_percentage = sum(accuracy_surprise) / n(),
            accuracy_n = n())

# fit Bayesian logistic regression 
fit.brm.exp2.order_effect = brm(formula = accuracy_surprise ~ 1 + index + condition,
                                data = df.exp2.order_effect,
                                family = "bernoulli",
                                seed = 1,
                                file = "cache/fit.brm.exp2.order_effect")  

# effects of condition and index
emm_order_effect_condition_index = emmeans(object = fit.brm.exp2.order_effect,
                                           specs = ~ index + condition)

# pairwise comparisons for index and condition
contrasts_condition_index = contrast(emm_order_effect_condition_index,
                                     method = "pairwise",
                                     simple = "each")

# overall effect of index
emm_order_effect_index = emmeans(object = fit.brm.exp2.order_effect,
                                 specs = ~ index)

# Conduct pairwise comparisons for index and condition
contrasts_index = contrast(emm_order_effect_index,
                           method = "pairwise")
contrasts_index
```
...
### Order effects in accuracy 

```{r}
df.exp2.order_effect %>% 
  mutate(condition = factor(condition, levels = c("long", "short", "conjunctive")),
         experiment = "2") %>% 
  group_by(experiment, condition, index) %>% 
  summarize(accuracy_percentage = sum(accuracy_surprise) / n(),
            accuracy_n = n()) %>% 
  rename(accuracy = accuracy_percentage,
         n = accuracy_n) %>% 
  relocate(experiment) %>% 
  print_table()
```
...
df.exp3.explanations = read_csv("data/explanations_coded.csv")
```
...
We predict that there will be a negative effect of training (people will be more likely to select an image with the cubes on the right side of the ramp when the ramp faces backwards than when it faces forwards). We predict that there will be a positive effect of generalization (people will be more likely to select an image with the cubes on the right side when the ramp in the generalization trial faces forward than when it faces backward). We predict that there will be a positive interaction effect (the difference in participants' selections between the two types of generalization trials will be greater when the ramp faced forward in the training compared to when it faced backward).
...
fit.features_full = lm(formula = prop ~ 1 + ramp_orientation_training * side * feature * friction,
                     data = df.features)
...
```{r}
fit.features_simple = lm(formula = prop ~ 1 + ramp_orientation_training * (side + feature + friction),
                         data = df.features)
...
```{r}
df.exp3.order_effect  = df.exp3.trials %>% 
  mutate(end_position = ifelse(is.na(end_position), end_posision, end_position)) %>% 
  select(workerid = prolific_id, trial, end_position, accuracy) %>% 
  group_by(workerid, trial) %>% 
  mutate(order = 1:n()) %>% 
  ungroup() %>% 
  filter((trial == "prediction_trial" & order == 4) |
  ...
  rename(prediction = end_position_prediction_trial,
         ...
         accuracy_surprise = accuracy_surprise_quiz) %>% 
  mutate(condition = "long",
         index = ifelse(prediction == surprise, "same", "different"))

df.exp3.order_effect %>% 
  group_by(condition, index) %>% 
  summarize(accuracy_percentage = sum(accuracy_surprise) / n(),
            accuracy_n = n())

fit.brm.exp3.order_effect = brm(formula = accuracy_surprise ~ 1 + index,
                            data = df.exp3.order_effect,
                            family = "bernoulli",
                            seed = 1,
                            file = "cache/fit.brm.exp3.order_effect")  

fit.brm.exp3.order_effect %>% 
  summary()

# overall effect of index
emm_order_effect_index = emmeans(object = fit.brm.exp3.order_effect,
                                 specs = ~ index)
...
```{r eval=FALSE}
df.exp3.explanations = df.exp3 %>%
  filter(trial == "generalization_check_survey") %>%
  select(prolific_id, ramp_orientation, ramp_or_cube, orientation,
         predicted_further, predicted_direction, response) %>%
  mutate(response = map_chr(response, ~ fromJSON(.)$response))

# write_csv(df.exp3.explanations,
#           file = "explanations.csv")
```

#### Explanation coding analysis 

```{r}
# mechanism information 
df.exp3.explanations %>%
  count(motion_mechanism) %>%
...
df.exp3.explanations %>% 
  ...
  mutate(prop = 100*n/sum(n)) %>% 
  select(-n) %>% 
  mutate(feature = factor(feature, levels = c("both", "cube_feature", "ramp_feature", "neither"))) %>% 
  pivot_wider(names_from = feature,
              values_from = prop) %>% 
  print_table(digits = 0)

# features by whether ramp or cube mattered
df.exp3.explanations %>% 
  group_by(ramp_or_cube) %>%
  count(feature) %>%
  mutate(prop = 100*n/sum(n)) %>% 
  select(-n) %>% 
  mutate(feature = factor(feature, levels = c("both", "cube_feature", "ramp_feature", "neither"))) %>% 
  arrange(ramp_or_cube, feature) %>% 
  pivot_wider(names_from = feature,
              values_from = prop) %>% 
  print_table(digits = 0)
```
...
df.exp3.order_effect %>% 
  ...
  mutate(condition = " ",
         experiment = "3",) %>% 
  select(experiment, condition, everything()) %>% 
  print_table()
```
...
df.title = df.model %>% 
  distinct(ramp_orientation_absolute, stimulus) %>% 
  mutate(title = ifelse(stimulus == "ramp",
                        str_c("old ramps ", ramp_orientation_absolute, ", new cubes"),
                        str_c("new ramp ", ramp_orientation_absolute, ", old cubes")))
