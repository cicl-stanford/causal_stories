"""Optimize p, q, beta, r against participant choices."""
import os
import numpy as np
import pandas as pd
import glob
...
from run_paths import KDE_DIR, PHYSICS_DIR, BESTFIT_DIR, print_progress
...
def load_participant_data(show_details=False):
    ...
    participant_file = os.path.join(
        _REPO_DIR, "R", "cache", "exp3_generalization_results.csv"
    )
...
def optimize_parameters(kde_results, participant_data, seed=1, show_details=False, n_restarts=10, verbose=True):
    ...
    # Generate random restarts from a fixed seed sequence
    rng = np.random.default_rng(seed)
    for _ in range(n_restarts):
        p_random = rng.uniform(P_BOUNDS[0], P_BOUNDS[1])
        q_random = rng.uniform(Q_BOUNDS[0], Q_BOUNDS[1])
        beta_random = 10 ** rng.uniform(np.log10(BETA_BOUNDS[0]), np.log10(BETA_BOUNDS[1]))
        r_random = rng.uniform(R_BOUNDS[0], R_BOUNDS[1])
...
def save_best_fit_results(best_result, predictions, participant_data, kde_file_info):
    ...
    try:
        ...
        params_file = os.path.join(BESTFIT_DIR, 'best_parameters.csv')
        ...
        if predictions is not None:
            ...
            comparison_file = os.path.join(BESTFIT_DIR, 'model_vs_human_comparison.csv')
            ...
            kde_dest = os.path.join(BESTFIT_DIR, os.path.basename(kde_source))
            ...
            if match:
                ...
                physics_file = os.path.join(
                    PHYSICS_DIR, f"trial_results_blk{block_noise}_rmp{ramp_noise}.csv"
                )
                physics_dest = os.path.join(BESTFIT_DIR, os.path.basename(physics_file))
        ...
        print(f"\nBest fit results saved to {BESTFIT_DIR}!")
...
def optimize_single_kde_file(kde_file, participant_data, save_results=True, save_to_bestfit=True, verbose=True):
    ...
    try:
        ...
        opt_results = optimize_parameters(kde_results, participant_data, seed=int(seed) if seed != 'unknown' else 1, n_restarts=N_RANDOM_RESTARTS, verbose=verbose)
        ...
        if opt_results['status'] == 'success':
            ...
            if save_results:
                ...
                output_file = os.path.join(
                    KDE_DIR,
                    f"optimized_parameters_blk{block_noise}_rmp{ramp_noise}_bw{bandwidth}_seed{seed}_{timestamp}.csv",
                )
                ...
                if opt_results['predictions'] is not None:
                    predictions_file = os.path.join(
                        KDE_DIR,
                        f"optimized_predictions_blk{block_noise}_rmp{ramp_noise}_bw{bandwidth}_seed{seed}_{timestamp}.csv",
                    )
...
def _optimize_file_worker(payload):
    """Multiprocessing worker: optimize one KDE file. Must be top-level for spawn."""
    kde_file, participant_data = payload
    result = optimize_single_kde_file(
        kde_file,
        participant_data,
        save_results=False,
        save_to_bestfit=False,
        verbose=False,
    )
    result.pop("optimization_result", None)
    return result


def optimize_all_kde_files(save_results=True, specific_files=None, workers=None):
    ...
    if specific_files:
        ...
        kde_pattern = os.path.join(KDE_DIR, "kde_results_*.csv")
        ...
        if not kde_files:
            ...
            print("  python step2_bayesian_inference.py")
    ...
    def handle_result(result, current_idx):
        nonlocal successful, failed, best_log_likelihood
        ...
        if best_log_likelihood != float("-inf"):
            extra = f"best {best_log_likelihood:.1f}"
        if failed:
            extra = f"{extra}  fail {failed}".strip()
        print_progress(current_idx, len(kde_files), start_time, label="fit", extra=extra)

    if workers <= 1:
        for i, kde_file in enumerate(kde_files, 1):
            result = optimize_single_kde_file(
                kde_file, participant_data, save_results=False, save_to_bestfit=False, verbose=False
            )
            handle_result(result, i)
    else:
        import multiprocessing as mp

        payloads = [(kde_file, participant_data) for kde_file in kde_files]
        n_proc = min(workers, len(payloads))
        with mp.Pool(processes=n_proc, maxtasksperchild=16) as pool:
            for i, result in enumerate(pool.imap_unordered(_optimize_file_worker, payloads, chunksize=1), 1):
                handle_result(result, i)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Step 3: optimize p, q, beta, r over KDE files")
    parser.add_argument(
        "--workers",
        type=int,
        default=None,
    ...
    optimize_all_kde_files(save_results=True, workers=args.workers)
